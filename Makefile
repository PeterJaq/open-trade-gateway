...
PERFTEST_NAME := open-trade-perftest
...
PERFTEST_SRCS:= $(wildcard open-trade-perftest/*.cpp)
...
PERFTEST_OBJS := $(patsubst open-trade-perftest/%,obj/perftest/%,$(PERFTEST_SRCS:.cpp=.o))
...
PERFTEST_DEPS := $(PERFTEST_OBJS:%.o=%.d)
...
all: bin/$(COMMON_NAME) bin/$(CTP_NAME) bin/$(CTPSE_NAME) bin/$(SIM_NAME) bin/$(PERFTEST_NAME) bin/$(MD_NAME) bin/$(GATEWAY_NAME)
...
bin/$(PERFTEST_NAME):$(PERFTEST_OBJS)
	@mkdir -p $(@D)
	$(CXX) -o $@ $(CXXFLAGS) $(LDFLAGS) $^ $(LDLIBS) 

obj/perftest/%.o: open-trade-perftest/%.cpp
	@mkdir -p $(@D)
	$(CXX) -o $@ -MMD -MP $(CPPFLAGS) $(CXXFLAGS) -c $<

-include $(PERFTEST_DEPS)
...
install: all
	...
	install -m 755 bin/$(PERFTEST_NAME) /usr/local/bin/
