...
void tradersim::ReceiveMsg()
{
	...
	while (true)
	{
		try
		{
			...
			if (!flag)
			{				
				_ios.post(boost::bind(&tradersim::SendUserDataImd, this));
			...
			if ((nPos <= 0) || (nPos+1 >= line.length()))
			{
				Log(LOG_WARNING
					, NULL
					, "trade perftest ReceiveMsg:%s is invalid!"
					, line.c_str());
				continue;
			}
			else
		...
		{
			Log(LOG_ERROR, NULL, "trade perftest ReceiveMsg_i Erro:%s", ex.what());
		}
	}
}
...
{	
	...
	Log(LOG_INFO, NULL,"trade perftest OnInit, UserID=%s",m_user_id.c_str());
}
...
{
	Log(LOG_INFO,NULL,"trade perftest CloseConnection:%d", nId);
...
void tradersim::ProcessInMsg(int connId,std::shared_ptr<std::string> msg_ptr)
{
	...
	{
		Log(LOG_WARNING, NULL, "trade perftest parse json(%s) fail", msg.c_str());
		return;
	}
	...
	else
	{
		if (!m_b_login)
		{
			Log(LOG_WARNING, NULL, "trade perftest receive other msg before login:%s"
				,msg.c_str());			
			return;
		}
		...
		if (!IsConnectionLogin(connId))
		{
			Log(LOG_WARNING, NULL
				,"trade perftest receive other msg which from not login connecion:%s"
				,msg.c_str());
			return;
		}
...
void tradersim::SendMsgAll(std::shared_ptr<std::string> msg_ptr)
{
	...
	if (totalLength > MAX_MSG_LENTH)
	{
		...
		{
			Log(LOG_ERROR, NULL, "trade perftest SendMsg Erro:%s,length:%d"
				, ex.what(), msg.length());
		}
	}
	else
	{
		...
		{
			Log(LOG_ERROR, NULL, "trade perftest SendMsg Erro:%s,msg:%s,length:%d"
